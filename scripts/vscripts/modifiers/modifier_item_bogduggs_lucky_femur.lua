...
function modifier_item_bogduggs_lucky_femur:OnAbilityFullyCast( params )
	if IsServer() then
		...
		if Ability:IsRefreshable() and Ability:IsItem() == false and RollPercentage( self.refresh_pct ) then
			Ability:EndCooldown()
			local nFXIndex = ParticleManager:CreateParticle( "particles/units/heroes/hero_ogre_magi/ogre_magi_bloodlust_buff_circle_flash.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent() )
